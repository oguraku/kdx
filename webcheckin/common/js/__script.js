    ...
    const swiperInstance = new Swiper(carouselEl, {
      ...
      on: {
        ...
        slideChange: function() {
          ...
          // 現在のスライドが disabled なら、有効なスライドを探して移動
          if (currentSlide && currentSlide.getAttribute('data-paxSeat') === 'disabled') {
            // 移動方向を判定（前回のインデックスから現在地への方向）
            const isForward = this.previousIndex < this.activeIndex;
            let validIndex = -1;
            
            if (isForward) {
              // 前方を探す
              for (let i = this.activeIndex + 1; i < this.slides.length; i++) {
                if (this.slides[i].getAttribute('data-paxSeat') !== 'disabled') {
                  validIndex = i;
                  break;
                }
              }
            ...
            if (validIndex >= 0) {
              // 有効なスライドが見つかったら、そこに直接移動（アニメーションなし、イベントなし）
              this.slideTo(validIndex, 0, false);
              return;
            }
          }
        ...
        slideChange: function() {
          ...
          if (currentSlide && currentSlide.getAttribute('data-paxSeat') === 'disabled') {
            ...
            if (validIndex >= 0) {
              ...
              updateOriginalSliderPosition(validIndex);
              // フォーカス制御を実行
              controlSlideFocus(this);
              return;
            }
